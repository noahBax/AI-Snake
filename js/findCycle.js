...
function findCycle() {
    ...
    if (ret.length == 0) {
        if (LOGGING)
            console.log("Searching for tail instead");
        ret = graphSearch(tail_node, true);
        if (ret.length == 0) {
            if (LOGGING)
                console.log("No valid search to tail");
        }
    }
...
function graphSearch(goal_node, going_to_tail) {
    let counter = 0;
    let final_paths;
    ...
    let deepest_path = 0;
    function searchPattern(pocket_invulnerable) {
        let opened = { 0: [] };
        let closed = {};
        final_paths = [];
        ...
        while (num_opened > 0) {
            ...
            if (counter > 18000 || BOARD_SIZE ** 2 - snake_length <= 3 && counter > 1500)
                break;
            ...
            for (const i in opened) {
                if (opened[i].length == 0)
                    continue;
                if (!best_node)
                    best_node = opened[i][0];
            ...
            if (best_node.source_node == goal_node) {
                ...
                if (snake_length > 4) {
                    ...
                    if (!see_goal && LOGGING)
                        console.log("Here's fail", best_node.trimSnake(), best_node.compileInstructions());
    ...
    if (final_paths.length == 0) {
        ...
        for (const d in DIRECTION) {
            if (BOARD_GRAPH[snake_head_cell.location_as_index].neighbors[d] == snake_tail_cell) {
                if (LOGGING)
                    console.log("Neighbors to the goal");
                return [DIRECTION[d]];
            }
        }
        return [];
    ...
    const best_path = final_paths[0][1];
