...
class SnakePathingNode {
    ...
    goal_node;
    is_original;
    g; // Length of current travel
    h; // Estimated length to end
    t; // Number of turns
    f; // Sum of heuristics
    visited = [];
    location_as_index;
    constructor(origin_node, g_cost_or_next, end_node, direction, prev_direction, next_node, is_original = false) {
        if (origin_node instanceof GraphNode) {
            ...
            if (next_node)
                this.visited.push(...next_node.visited);
            this.visited.splice(snake_length);
            this.t = 0;
            if (next_node)
                this.t = next_node.t;
            if (next_node && this.direction != next_node.direction)
                this.t++;
            this.g = g_cost_or_next;
            // Manhattan distance
    ...
    findCurrentTail(do_along_the_way, just_behind = false) {
        let curr_tail = this;
        let layers_to_drop = snake_length - 1;
        while (layers_to_drop > 0) {
            if (do_along_the_way)
                do_along_the_way(curr_tail);
            layers_to_drop--;
    ...
    getNeighbors(opened, closed, one_step_in_future = false) {
        const ret = [];
        function checkNeighbor(snake_node, i) {
            let neighbor = snake_node.source_node.neighbors[i];
            // The neighbor doesn't exist or the neighbor is currently part of the snake
            if (!neighbor || snake_node.checkIsSnake(neighbor, one_step_in_future))
                return false;
    ...
    canTailSeeHeadWithSpace(pathing_to_tail, pocket_invulnerable) {
        ...
        if (!post_tail || post_tail.is_snake)
            post_tail = tail_node.cellBehindSelf(false);
        ...
        if (!pathing_to_tail && snake_left > 2 && in_pocket.length == 0) {
            can_see = false;
            if (LOGGING)
                console.log("Too few cells in pocket");
        }
        // if (game_slow) await delay(50);
        if (tail_ele == head) {
            can_see = false;
            if (LOGGING)
                console.log("Tail is head");
        }
        if (!pathing_to_tail && post_tail == head && snake_left > 1) {
            can_see = false;
            if (LOGGING)
                console.log("Head is in poop");
        }
        if (!pathing_to_tail) {
            for (const i in tail_ele.neighbors) {
                if (tail_ele.neighbors[i]?.location_as_index == head.location_as_index) {
                    can_see = false;
                    if (LOGGING)
                        console.log("Tail is neighbor of head");
                }
            }
        }
        ...
        if (can_see && snake_left > 1 && !pocket_invulnerable) {
            ...
            for (const g of BOARD_GRAPH) {
                if (g.is_snake || g.seeable_neighbors > 0)
                    continue;
                pocket_count++;
                to_look_at.push(g);
                ...
                if (in_pocket.length + pocket_gain == 2)
                    size_2_pockets++;
                else if (in_pocket.length + pocket_gain == 1)
                    size_1_pockets++;
            ...
            if (LOGGING)
                console.log("Pocket situtation", pocket_collection);
            if (can_see && LOGGING)
                console.log("Pockets sufficient given length");
            else {
                if (LOGGING)
                    console.log("Too many pockets", pocket_count);
            }
