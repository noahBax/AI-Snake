...
class AStarNode {
    ...
    source_node;
    is_snake = false;
    g = Infinity; // Cost from beginning to here
    h = Infinity; // Estimated cost from here to end
    next_node;
    get f() { return this.g + this.h; } // Sum of heuristics
    constructor(graph_node, g_cost, end_node, direction, is_snake = false) {
...
async function findHamiltononianCycle(direction) {
    let cycle_length = 0;
    const path = [];
...
async function astar(head_node, apple_node, direction) {
    let counter = 0;
    let final_paths = [];
    let closed = [];
    let opened = [];
    ...
    while (opened.length > 0) {
        ...
        if (curr_node.source_node == apple_node) {
            ...
            break;
            if (final_paths.length == 20)
                break;
        }
        // Loop through neighbors, calculate costs, update opened
        for (const i in curr_node.source_node.neighbors) {
            if (!curr_node.source_node.neighbors[i])
                continue;
            let neighbor = curr_node.source_node.neighbors[i];
            // if (neighbor.visited) continue;
            if (neighbor.is_snake && neighbor.timer - curr_node.g > 0)
                continue;
            if (curr_node.visited_history.includes(neighbor))
                continue;
