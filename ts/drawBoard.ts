import { path_ctx, snake_head_cell, TICK_RATE, apple, addTail, spawnApple, ctx, sprite_sheet, TILE_WIDTH, snake_length, BOARD_SIZE, game_slow, delay, game_paused, snake_tail_cell, LOGGING } from "./index.js";
...
function drawBoard(time: number) {
	...
	if (!game_over && snake_head_cell.location_as_index == apple.location_as_index) {
		...
		if (snake_length < BOARD_SIZE * BOARD_SIZE) {
			...
			const moves = findCycle();
			if (LOGGING) console.log(`${moves.length} moves received`);
			if (LOGGING) console.log([...moves]);
			if (moves.length == 0) {
				if (LOGGING) console.log("fails");
				game_over = true;
			} else {
				move_queue = moves;
				window.move_queue = move_queue;
			...
			game_over = true;
			game_win = true;
			if (LOGGING) console.log("U R Winner");
		}
	}
